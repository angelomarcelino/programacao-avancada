...
{
  SOCKET id_prov(id);
  MYSOCKET_STATE state_prov(state);
  id = S.id;
  state = S.state;
  S.id = id_prov;
  S.state = state_prov;
...
MYSOCKET_STATUS mysocket::shutdown()
{
  MYSOCKET_STATUS iResult;

  if (id != INVALID_SOCKET && state != MYSOCKET_IDLE) {
    state = MYSOCKET_IDLE;
...
std::ostream& operator<<(std::ostream& os, const mysocket &s)
{
  ...
  case MYSOCKET_IDLE:
    os << 'I';
    break;
  case MYSOCKET_ACCEPTING:
    os << 'A';
    break;
  case MYSOCKET_CONNECTED:
    os << 'C';
    break;
  }
  os << ')';
...
MYSOCKET_STATUS tcp_mysocket::read(char *dado, size_t len, long milisec) const
{
  ...
  do
  {
    ...
    if ( ultima_leitura == SOCKET_ERROR || ultima_leitura == 0 )
    {
      // Deu erro ou servidor desconectou
      return SOCKET_ERROR;
    }
...
MYSOCKET_STATUS tcp_mysocket_server::listen(const char *port, int nconex)
{
    ...
    if (iResult != 0) {
        cerr << "mysocket listen - getaddrinfo failed: " << iResult << endl;
        return(SOCKET_ERROR);
    }
    ...
    id = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
    if (id == INVALID_SOCKET) {
        cerr << "mysocket listen - error at socket(): " << WSAGetLastError() << endl;
        freeaddrinfo(result);
        return(SOCKET_ERROR);
    }
    ...
    if (iResult == SOCKET_ERROR) {
        cerr << "mysocket listen - bind failed with error: " << WSAGetLastError() << endl;
        freeaddrinfo(result);
        close();
        return iResult;
    }
...
ostream& operator<<(ostream& os, const mysocket_queue &f)
{
    os << '[';
