...
class Usuario {
    ...
    public:
        ...
        inline const string& getLogin() const {
            return login;
        }
        ...
        inline tcp_mysocket& getSocket() {
            return s;
        }
        inline bool setSocket(const tcp_mysocket &S) {
            s=S;
            return true;
        }

        inline int32_t getLastId() const {
            return last_id;
        }
        bool setLastId(int32_t ID);

        /// Teste de igualdade com uma string (testa se a string eh igual ao login)
        inline bool operator==(const string &L) const {
            return login==L;
        }

        MYSOCKET_STATUS read_usuario(tcp_mysocket t, long milisec=-1);
