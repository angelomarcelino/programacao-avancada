...
void DadosCliente::findIndexMensagem(int32_t id, int& ind_conv, int& ind_msg) const
{
  ...
  for (ind_conv=0; !encontrou && ind_conv<(int)LC.size(); ind_conv++)
  {
    for (ind_msg=0; !encontrou && ind_msg<(int)LC[ind_conv].size(); ind_msg++)
    {
      encontrou = (LC[ind_conv][ind_msg].getId() == id);
    }
  }
  if (!encontrou)
  {
