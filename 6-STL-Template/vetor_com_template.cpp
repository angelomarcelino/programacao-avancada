...
template <class TT> /// TT é o termo genérico do template
class vector
{
private:
    unsigned N;
...
template <class TT>
void vector<TT>::copy(const vector<TT> &V) ///Precisa usar o nome completo clss_name<type_name>::func
{
    N = V.N;
    x = (N==0 ? NULL : new TT[N]);
...
/// ^ isso tudo é a mema coisa que #include <vector>

int main()
{
    /// Como criar um vector de float
    vector<float> Z;
